# -*- coding: utf-8 -*-
"""
Created on Tue Dec 13 08:50:37 2022
...
#This compares the 2 elements
def Comparer(el_1 , el_2):
    if isinstance(el_1, int):
        if isinstance(el_2,int):
            #If both are ints
            if el_1<el_2:
                return True
            elif el_1 > el_2:
            ...
            return Comparer([el_1],el_2)
    else:
        if isinstance(el_2,int):
            #First is list, 2nd in int
            return Comparer(el_1,[el_2])
        else:
            if el_1==[]:
                if el_2!=[]:
                #Reach the end of first list, if we have the end of 2nd, then true
                    return True
                else: 
                    return 'D'
            elif el_2==[]:
                ...
                # Compare first element of each lsit
                o = Comparer(el_1[0],el_2[0])
                if o=='D':
                    #If it's a draw, compare the rest
                    return Comparer(el_1[1:],el_2[1:])
                else:
                    return o
...
# Create a mergesort algorithm for this


def My_Merge(in_list):
    l = len(in_list)
    if l == 1:
        return [in_list]
    elif l == 2:
        return My_Zip([in_list[0]],[in_list[1]])
    else:
        y = in_list[:int(l / 2)]
        z = in_list[int(l / 2):]
        m=My_Merge(y)
...
def My_Zip(c,d):
    ...
    while i1 < l1 and i2 < l2:
        x=Comparer(c[i1],d[i2])   
        if x==True:
            out_list.append(c[i1])
            i1 += 1
        else:
            out_list.append(d[i2])
            i2 +=1
    if i1<l1:
        for m in range(i1, l1):
            out_list.append(c[m])
    else: 
        for n in range(i2, l2):
            out_list.append(d[n])
    return out_list

my_sorted=My_Merge(my_in)
